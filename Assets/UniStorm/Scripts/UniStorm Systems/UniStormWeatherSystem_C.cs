//UniStorm Weather System C# Version 1.8.3 @ Copyright
	...
	*/
	...
	void WeatherForecaster () 
	{
		...
		if (weatherForecaster == 1)
		{
			...
			fade2 += Time.deltaTime * .04f;
			butterfliesFade -= .04f;
			windyLeavesFade -= .04f;
			fadeStormClouds -= Time.deltaTime * .04f;
			windControl += Time.deltaTime * .01f;
			...
			RenderSettings.fogDensity += .00017f * Time.deltaTime;
			
			if (RenderSettings.fogDensity >= .002)
			{
				RenderSettings.fogDensity = .002f;
			}
			
			heavyClouds.GetComponent<Renderer>().material.color = new Color(0.5f,0.5f,0.5f,fade);
			heavyCloudsLayer1.GetComponent<Renderer>().material.color = new Color(0,0,0,fade2);	
			heavyCloudsLayerLight.GetComponent<Renderer>().material.color = new Color(.6f,.6f,.6f,fade2);	
			...
			//Fades our fog
			fogScript = cameraThing.GetComponent<Camera>().GetComponent<GlobalFog>();
			fogScript.globalDensity += .0005f * Time.deltaTime;
			fogScript.startDistance -= 5 * Time.deltaTime;
			
			
			if (fogScript.globalDensity >= .007)
			{
				fogScript.globalDensity = .007f;
			}
			...
			//GetComponent<UniStormThunder_C>().enabled = false;
			...
			if (minRainIntensity <= 0)
			{
				...
				if (windZone.transform.eulerAngles.y <= 0)
				{
					...
					windZone.transform.eulerAngles = new Vector3(0, 1, 0);
				}
		...
		if (weatherForecaster == 2)
		{
			...
			if (temperature <= 32 && temperatureType == 1  || temperatureType == 2 && temperature <= 0)
			{
				...
				stormClouds += Time.deltaTime * .011f;
				windSnowSoundHandler += Time.deltaTime * .01f;
				dynamicSnowFade += Time.deltaTime * .0008f;
				...
				heavyClouds.GetComponent<Renderer>().material.color = new Color(.5f,.5f,.5f,fade);
				heavyCloudsLayer1.GetComponent<Renderer>().material.color = new Color(0,0,0,fade2);	
				heavyCloudsLayerLight.GetComponent<Renderer>().material.color = new Color(.6f,.6f,.6f,fade2);
				...
				if (RenderSettings.fogDensity <= .0006)
				{
					...
					fogScript = cameraThing.GetComponent<Camera>().GetComponent<GlobalFog>();
					fogScript.globalDensity += .0008f * Time.deltaTime;
					fogScript.startDistance -= 5 * Time.deltaTime;
				...
				if (overrideFog == true)
				{
					fogScript.globalDensity -= .001f * Time.deltaTime;
			...
			if (temperature >= 33 && temperatureType == 1 || temperatureType == 2 && temperature >= 1)
			{
				...
				fadeStormClouds += Time.deltaTime * .04f;
				fadeHorizon -= .4f;
				windControl += Time.deltaTime * .009f;
				time += Time.deltaTime * .14f;
				sunShaftFade -= Time.deltaTime * .14f;
				sunIntensity -= .005f;
				minRainIntensity += .2f;
				...
				RenderSettings.fogDensity -= .0005f * Time.deltaTime;
		...
		if (weatherForecaster == 3)
		{
			if (temperature <= 32 && temperatureType == 1 || temperatureType == 2 && temperature <= 0)
			{
				...
				minSnowIntensity += .4f;
				minFogIntensity -= .008f;
				minSnowFogIntensity += .01f;	
				stormClouds += Time.deltaTime * .011f;
				dynamicSnowFade += Time.deltaTime * .0012f;
				sunIntensity -= .005f;
			...
			if (temperature >= 33 && temperatureType == 1 || temperatureType == 2 && temperature >= 1)
			{
				...
				//Acivates UniStorm system
				//GetComponent<UniStormThunder_C>().enabled = true;
